...
public class RiotApiServiceImpl implements  RiotApiService{
    ...
    @Autowired
    public RiotApiServiceImpl(RestTemplateBuilder builder){
    ...
    private String summonerApiURL(String summonerId){
        return summonerByNameApiURL + encodeUrl(summonerId) + "?api_key=" + apiKey;
    }

    public List<String> fetchMatchIds(String puuid){
        return restTemplate
                .exchange(
                        matchPuuidApiURL(puuid),
                        HttpMethod.GET,
                        null,
